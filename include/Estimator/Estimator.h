...
public:
    ...
    static const int SLIDE_WINDOW_SIZE = 2;
    ...
    struct FeatureLine {
        ...
        double ComputeError(const Eigen::Matrix4d& pose) {
            ...
            double l12 = std::sqrt(
                (lineP1(0) - lineP2(0)) * (lineP1(0) - lineP2(0))
            ...
            // @note: why there is no return value
            return {};
        }
    };
    ...
    struct FeaturePlan {
        ...
        bool valid;
        FeaturePlan(
            Eigen::Vector3d po, const double& pa_, const double& pb_, const double& pc_, const double& pd_)
            : pointOri(std::move(po))
            , pa(pa_)
            , pb(pb_)
            , pc(pc_)
    ...
    struct FeaturePlanVec {
        ...
        double error;
        bool valid;
        FeaturePlanVec(Eigen::Vector3d po, Eigen::Vector3d p_proj, Eigen::Matrix3d sqrt_info_)
            : pointOri(std::move(po))
            , pointProj(std::move(p_proj))
            , sqrt_info(std::move(sqrt_info_)) {
            valid = false;
            error = 0;
        }
        double ComputeError(const Eigen::Matrix4d& pose) {
...
public:
    ...
    void processPointToLine(
        ...
        const pcl::KdTreeFLANN<PointType>::Ptr& kdtree, const Eigen::Matrix4d& exTlb,
    ...
    /** \brief estimate lidar pose by matching current lidar cloud with map cloud and tightly coupled IMU
     * message \param[in] lidarFrameList: multi-frames of lidar cloud and lidar pose \param[in] exTlb:
     * extrinsic matrix between lidar and IMU \param[in] gravity: gravity vector
     */
    ...
    pcl::PointCloud<PointType>::Ptr get_corner_map() { return map_manager->get_corner_map(); }
    pcl::PointCloud<PointType>::Ptr get_surf_map() { return map_manager->get_surf_map(); }
    pcl::PointCloud<PointType>::Ptr get_nonfeature_map() { return map_manager->get_nonfeature_map(); }
...
private:
    ...
    double para_PR[SLIDE_WINDOW_SIZE][6];
    double para_VBias[SLIDE_WINDOW_SIZE][9];
