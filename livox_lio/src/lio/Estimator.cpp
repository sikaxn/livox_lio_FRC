...
Estimator::Estimator(const float& filter_corner, const float& filter_surf) {
    laserCloudCornerFromLocal     = std::make_shared<pcl::PointCloud<PointType>>();
    laserCloudSurfFromLocal       = std::make_shared<pcl::PointCloud<PointType>>();
    laserCloudNonFeatureFromLocal = std::make_shared<pcl::PointCloud<PointType>>();

    laserCloudCornerLast.resize(SLIDE_WINDOW_SIZE);
    for (auto& p : laserCloudCornerLast)
        p = std::make_shared<pcl::PointCloud<PointType>>();

    laserCloudSurfLast.resize(SLIDE_WINDOW_SIZE);
    for (auto& p : laserCloudSurfLast)
        p = std::make_shared<pcl::PointCloud<PointType>>();

    laserCloudNonFeatureLast.resize(SLIDE_WINDOW_SIZE);
    for (auto& p : laserCloudNonFeatureLast)
        p = std::make_shared<pcl::PointCloud<PointType>>();

    laserCloudCornerStack.resize(SLIDE_WINDOW_SIZE);
    for (auto& p : laserCloudCornerStack)
        p = std::make_shared<pcl::PointCloud<PointType>>();

    laserCloudSurfStack.resize(SLIDE_WINDOW_SIZE);
    for (auto& p : laserCloudSurfStack)
        p = std::make_shared<pcl::PointCloud<PointType>>();

    laserCloudNonFeatureStack.resize(SLIDE_WINDOW_SIZE);
    for (auto& p : laserCloudNonFeatureStack)
        p = std::make_shared<pcl::PointCloud<PointType>>();

    laserCloudCornerForMap     = std::make_shared<pcl::PointCloud<PointType>>();
    laserCloudSurfForMap       = std::make_shared<pcl::PointCloud<PointType>>();
    laserCloudNonFeatureForMap = std::make_shared<pcl::PointCloud<PointType>>();

    transformForMap.setIdentity();

    kdtreeCornerFromLocal     = std::make_shared<pcl::KdTreeFLANN<PointType>>();
    kdtreeSurfFromLocal       = std::make_shared<pcl::KdTreeFLANN<PointType>>();
    kdtreeNonFeatureFromLocal = std::make_shared<pcl::KdTreeFLANN<PointType>>();

    for (int i = 0; i < localMapWindowSize; i++) {
        localCornerMap[i]     = std::make_shared<pcl::PointCloud<PointType>>();
        localSurfMap[i]       = std::make_shared<pcl::PointCloud<PointType>>();
        localNonFeatureMap[i] = std::make_shared<pcl::PointCloud<PointType>>();
    }
...
Estimator::~Estimator() {
    // auto non_feature = map_manager->get_nonfeature_map();
    // auto corner      = map_manager->get_corner_map();
    // auto surf        = map_manager->get_surf_map();

    // using PointCloudT = std::remove_reference_t<decltype(*corner)>;

    // auto pcd = std::make_shared<PointCloudT>();

    // *pcd += *non_feature;
    // *pcd += *corner;
    // *pcd += *surf;

    // const std::string path{"/workspaces/sentry/ignore/develop_ws/map.pcd"};
    // std::cout << "save map in " << path << std::endl;

    // pcl::io::savePCDFileASCII(path, *pcd);
    delete map_manager;
}
    ...
    while (true) {
        ...
        if (!laserCloudCornerForMap->empty()) {
            ...
            map_manager->featureAssociateToMap(
                laserCloudCornerForMap, laserCloudSurfForMap, laserCloudNonFeatureForMap, laserCloudCorner,
                laserCloudSurf, laserCloudNonFeature, transformForMap);
            ...
            *laserCloudNonFeature_to_map += *laserCloudNonFeature;
            ...
            if (map_update_ID % map_skip_frame == 0) {
                map_manager->MapIncrement(
                    laserCloudCorner_to_map, laserCloudSurf_to_map, laserCloudNonFeature_to_map, transform);
...
void Estimator::processPointToLine(
    std::vector<ceres::CostFunction*>& edges, std::vector<FeatureLine>& vLineFeatures,
    const pcl::PointCloud<PointType>::Ptr& laserCloudCorner,
    const pcl::PointCloud<PointType>::Ptr& laserCloudCornerLocal,
    const pcl::KdTreeFLANN<PointType>::Ptr& kdtreeLocal, const Eigen::Matrix4d& exTlb,
    ...
    if (!vLineFeatures.empty()) {
        for (const auto& l : vLineFeatures) {
            auto* e = Cost_NavState_IMU_Line::Create(
                l.pointOri, l.lineP1, l.lineP2, Tbl, Eigen::Matrix<double, 1, 1>(1 / IMUIntegrator::lidar_m));
            edges.push_back(e);
        }
        return;
    }
    ...
    for (int i = 0; i < laserCloudCornerStackNum; i++) {
        ...
        int id = map_manager->FindUsedCornerMap(
            &_pointSel, laserCenWidth_last, laserCenHeight_last, laserCenDepth_last);
        ...
        if (GlobalCornerMap[id].points.size() > 100) {
            ...
            if (_pointSearchSqDis[4] < thres_dist) {
                ...
                if (saes.eigenvalues()[2] > 3 * saes.eigenvalues()[1]) {
                    ...
                    auto* e = Cost_NavState_IMU_Line::Create(
                        Eigen::Vector3d(_pointOri.x, _pointOri.y, _pointOri.z), tripod1, tripod2, Tbl,
                        Eigen::Matrix<double, 1, 1>(1 / IMUIntegrator::lidar_m));
                    edges.push_back(e);
                    vLineFeatures.emplace_back(
                        Eigen::Vector3d(_pointOri.x, _pointOri.y, _pointOri.z), tripod1, tripod2);
...
void Estimator::processPointToPlan(
    std::vector<ceres::CostFunction*>& edges, std::vector<FeaturePlan>& vPlanFeatures,
    ...
    for (int i = 0; i < laserCloudSurfStackNum; i++) {
        ...
        int id = map_manager->FindUsedSurfMap(
            &_pointSel, laserCenWidth_last, laserCenHeight_last, laserCenDepth_last);
        ...
        if (GlobalSurfMap[id].points.size() > 50) {
            ...
            if (_pointSearchSqDis[4] < 1.0) {
                ...
                for (int j = 0; j < 5; j++) {
                    if (std::fabs(
                            pa * GlobalSurfMap[id].points[_pointSearchInd[j]].x
                            + pb * GlobalSurfMap[id].points[_pointSearchInd[j]].y
                            + pc * GlobalSurfMap[id].points[_pointSearchInd[j]].z + pd)
                        > 0.2) {
                        planeValid = false;
                        break;
                    }
                ...
                if (planeValid) {
                    debug_num12++;
                    auto* e = Cost_NavState_IMU_Plan::Create(
                        Eigen::Vector3d(_pointOri.x, _pointOri.y, _pointOri.z), pa, pb, pc, pd, Tbl,
                        Eigen::Matrix<double, 1, 1>(1 / IMUIntegrator::lidar_m));
                    edges.push_back(e);
                    vPlanFeatures.emplace_back(
                        Eigen::Vector3d(_pointOri.x, _pointOri.y, _pointOri.z), pa, pb, pc, pd);
            ...
            if (_pointSearchSqDis2[4] < 1.0) {
                ...
                for (int j = 0; j < 5; j++) {
                    if (std::fabs(
                            ...
                            + pc * laserCloudSurfLocal->points[_pointSearchInd2[j]].z + pd)
                        > 0.2) {
                        planeValid = false;
                        break;
                    }
...
void Estimator::processPointToPlanVec(
    std::vector<ceres::CostFunction*>& edges, std::vector<FeaturePlanVec>& vPlanFeatures,
    const pcl::PointCloud<PointType>::Ptr& laserCloudSurf,
    const pcl::PointCloud<PointType>::Ptr& laserCloudSurfLocal,
    const pcl::KdTreeFLANN<PointType>::Ptr& kdtreeLocal, const Eigen::Matrix4d& exTlb,
    const Eigen::Matrix4d& m4d) {
    ...
    if (!vPlanFeatures.empty()) {
        for (const auto& p : vPlanFeatures) {
            auto* e = Cost_NavState_IMU_Plan_Vec::Create(p.pointOri, p.pointProj, Tbl, p.sqrt_info);
            edges.push_back(e);
        }
        return;
    }
            ...
            if (_pointSearchSqDis[4] < thres_dist) {
                ...
                if (planeValid) {
                    ...
                    Eigen::Vector3d point_proj =
                        Eigen::Vector3d(_pointSel.x, _pointSel.y, _pointSel.z) - (dist * omega);
                    ...
                    auto* e = Cost_NavState_IMU_Plan_Vec::Create(
                        Eigen::Vector3d(_pointOri.x, _pointOri.y, _pointOri.z), point_proj, Tbl, sqrt_info);
                    edges.push_back(e);
                    vPlanFeatures.emplace_back(
                        Eigen::Vector3d(_pointOri.x, _pointOri.y, _pointOri.z), point_proj, sqrt_info);
                    vPlanFeatures.back().ComputeError(m4d);
...
void Estimator::processNonFeatureICP(
    std::vector<ceres::CostFunction*>& edges, std::vector<FeatureNon>& vNonFeatures,
    ...
    for (int i = 0; i < laserCloudNonFeatureStackNum; i++) {
            ...
            if (_pointSearchSqDis[4] < 1 * thres_dist) {
                ...
                for (int j = 0; j < 5; j++) {
                    if (std::fabs(
                            ...
                            + pc * GlobalNonFeatureMap[id].points[_pointSearchInd[j]].z + pd)
                        > 0.2) {
                        planeValid = false;
                        break;
                    }
        ...
        if (laserCloudNonFeatureLocal->points.size() > 20) {
            ...
            if (_pointSearchSqDis2[4] < 1 * thres_dist) {
                ...
                for (int j = 0; j < 5; j++) {
                    if (std::fabs(
                            pa * laserCloudNonFeatureLocal->points[_pointSearchInd2[j]].x
                            + pb * laserCloudNonFeatureLocal->points[_pointSearchInd2[j]].y
                            + pc * laserCloudNonFeatureLocal->points[_pointSearchInd2[j]].z + pd)
                        > 0.2) {
                        planeValid = false;
                        break;
                    }
...
void Estimator::EstimateLidarPose(
    std::list<LidarFrame>& lidarFrameList, const Eigen::Matrix4d& exTlb, const Eigen::Vector3d& gravity,
    ...
    auto laserCloudCornerFromMapNum   = map_manager->get_corner_map()->points.size();
    auto laserCloudSurfFromMapNum     = map_manager->get_surf_map()->points.size();
    auto laserCloudCornerFromLocalNum = laserCloudCornerFromLocal->points.size();
    auto laserCloudSurfFromLocalNum   = laserCloudSurfFromLocal->points.size();
    int stack_count                   = 0;
    for (const auto& l : lidarFrameList) {
    ...
    *laserCloudNonFeatureForMap = *laserCloudNonFeatureStack[0];
    ...
    MapIncrementLocal(
        laserCloudCornerForMap, laserCloudSurfForMap, laserCloudNonFeatureForMap, transformTobeMapped);
    locker.unlock();
}
...
void Estimator::Estimate(
    ...
    auto windowSize                     = lidarFrameList.size();
    Eigen::Matrix4d transformTobeMapped = Eigen::Matrix4d::Identity();
    Eigen::Matrix3d exRbl               = exTlb.topLeftCorner(3, 3).transpose();
    Eigen::Vector3d exPbl               = -1.0 * exRbl * exTlb.topRightCorner(3, 1);
    ...
    if (windowSize == SLIDE_WINDOW_SIZE) {
    ...
    for (int iterOpt = 0; iterOpt < max_iters; ++iterOpt) {
            ...
            problem.AddResidualBlock(
                Cost_NavState_PRV_Bias::Create(
                    ...
                    Eigen::LLT<Eigen::Matrix<double, 15, 15>>(
                        ...
                        .matrixL()
                        .transpose()),
                nullptr, para_PR[f - 1], para_VBias[f - 1], para_PR[f], para_VBias[f]);
        }
        ...
        for (int f = 0; f < windowSize; ++f) {
            ...
            threads[0] = std::thread(
                &Estimator::processPointToLine, this, std::ref(edgesLine[f]), std::ref(vLineFeatures[f]),
                std::ref(laserCloudCornerStack[f]), std::ref(laserCloudCornerFromLocal),
                std::ref(kdtreeCornerFromLocal), std::ref(exTlb), std::ref(transformTobeMapped));

            threads[1] = std::thread(
                &Estimator::processPointToPlanVec, this, std::ref(edgesPlan[f]), std::ref(vPlanFeatures[f]),
                std::ref(laserCloudSurfStack[f]), std::ref(laserCloudSurfFromLocal),
                std::ref(kdtreeSurfFromLocal), std::ref(exTlb), std::ref(transformTobeMapped));

            threads[2] = std::thread(
                &Estimator::processNonFeatureICP, this, std::ref(edgesNon[f]), std::ref(vNonFeatures[f]),
                std::ref(laserCloudNonFeatureStack[f]), std::ref(laserCloudNonFeatureFromLocal),
                std::ref(kdtreeNonFeatureFromLocal), std::ref(exTlb), std::ref(transformTobeMapped));
        ...
        if (deltaR < 0.05 && deltaT < 0.05 || (iterOpt + 1) == max_iters) {
            ...
            ceres::CostFunction* IMU_Cost = Cost_NavState_PRV_Bias::Create(
                ...
                Eigen::LLT<Eigen::Matrix<double, 15, 15>>(frame_curr->imuIntegrator.GetCovariance().inverse())
                    ...
                    .transpose());
            auto* residual_block_info = new ResidualBlockInfo(
                IMU_Cost, nullptr, std::vector<double*>{para_PR[0], para_VBias[0], para_PR[1], para_VBias[1]},
                std::vector<int>{0, 1});
            marginalization_info->addResidualBlockInfo(residual_block_info);
            ...
            for (auto& e : edgesLine[f]) {
                if (vLineFeatures[f][cntFtu].valid) {
                    auto* residual_block_info = new ResidualBlockInfo(
                        e, nullptr, std::vector<double*>{para_PR[0]}, std::vector<int>{0});
                    marginalization_info->addResidualBlockInfo(residual_block_info);
                }
                cntFtu++;
            }
...
void Estimator::MapIncrementLocal(
    ...
    auto laserCloudCornerStackNum     = laserCloudCornerStack->points.size();
    auto laserCloudSurfStackNum       = laserCloudSurfStack->points.size();
    auto laserCloudNonFeatureStackNum = laserCloudNonFeatureStack->points.size();
    PointType pointSel;
    PointType pointSel2;
    ...
    for (int i = 0; i < laserCloudNonFeatureStackNum; i++) {
        MAP_MANAGER::pointAssociateToMap(
            &laserCloudNonFeatureStack->points[i], &pointSel2, transformTobeMapped);
