...
void IMUIntegrator::PushIMUMsg(const sensor_msgs::msg::Imu::ConstSharedPtr& imu) { vimuMsg.push_back(imu); }
...
void IMUIntegrator::GyroIntegration(double lastTime) {
    ...
    for (auto& imu : vimuMsg) {
        ...
        double dt = get_time_sec(imu->header.stamp) - current_time;
        ...
        current_time = get_time_sec(imu->header.stamp);
    }
}
...
void IMUIntegrator::PreIntegration(double lastTime, const Eigen::Vector3d& bg, const Eigen::Vector3d& ba) {
    ...
    for (auto& imu : vimuMsg) {
        ...
        gyr << imu->angular_velocity.x, imu->angular_velocity.y, imu->angular_velocity.z;
        Eigen::Vector3d acc;
        acc << imu->linear_acceleration.x * gnorm, imu->linear_acceleration.y * gnorm,
            imu->linear_acceleration.z * gnorm;
        double dt = get_time_sec(imu->header.stamp) - current_time;

        if (dt <= 0)
            RCLCPP_WARN(rclcpp::get_logger("imu_integrator"), "dt <= 0");
        ...
        if (gyr_dt_norm > 0.00001) {
            ...
            Jr                = Eigen::Matrix3d::Identity() - (1 - cos(gyr_dt_norm)) / gyr_dt_norm * K
