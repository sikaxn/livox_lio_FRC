...
MAP_MANAGER::MAP_MANAGER(const float& filter_corner, const float& filter_surf) {
    ...
    for (int i = 0; i < laserCloudNum; i++) {
        laserCloudCornerArray[i]          = std::make_shared<pcl::PointCloud<PointType>>();
        laserCloudSurfArray[i]            = std::make_shared<pcl::PointCloud<PointType>>();
        laserCloudNonFeatureArray[i]      = std::make_shared<pcl::PointCloud<PointType>>();
        laserCloudCornerArrayStack[i]     = std::make_shared<pcl::PointCloud<PointType>>();
        laserCloudSurfArrayStack[i]       = std::make_shared<pcl::PointCloud<PointType>>();
        laserCloudNonFeatureArrayStack[i] = std::make_shared<pcl::PointCloud<PointType>>();

        laserCloudCornerKdMap[i]     = std::make_shared<pcl::KdTreeFLANN<PointType>>();
        laserCloudSurfKdMap[i]       = std::make_shared<pcl::KdTreeFLANN<PointType>>();
        laserCloudNonFeatureKdMap[i] = std::make_shared<pcl::KdTreeFLANN<PointType>>();
    }
    ...
    laserCloudCornerFromMap     = std::make_shared<pcl::PointCloud<PointType>>();
    laserCloudSurfFromMap       = std::make_shared<pcl::PointCloud<PointType>>();
    laserCloudNonFeatureFromMap = std::make_shared<pcl::PointCloud<PointType>>();
...
void MAP_MANAGER::pointAssociateToMap(
    PointType const* const pi, PointType* const po, const Eigen::Matrix4d& _transformTobeMapped) {
...
void MAP_MANAGER::MapIncrement(
    ...
    for (int i = 0; i < laserCloudCornerStackNum; i++) {
        ...
        if (cubeI >= 0 && cubeI < laserCloudDepth && cubeJ >= 0 && cubeJ < laserCloudWidth && cubeK >= 0
