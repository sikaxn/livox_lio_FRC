...
int frame_count = 0;
// 5 frames update
int frame_lenth_threshold = 5;

int GetNeiborPCA_cor(
    SNeiborPCA_cor& npca, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::KdTreeFLANN<pcl::PointXYZ> kdtree,
...
int FilterGndForPos_cor(float* outPoints, float* inPoints, int inNum) {
    ...
    for (int pid = 0; pid < inNum; pid++) {
        idtemp[pid] = -1;
        if ((inPoints[pid * 4] > -x_len) && (inPoints[pid * 4] < x_len) && (inPoints[pid * 4 + 1] > -y_len)
    ...
    for (int pid = 0; pid < inNum; pid++) {
        ...
        if (idtemp[pid] > 0 && idtemp[pid] < nx * ny) {
            ...
            if ((imgMaxZ[idtemp[pid]] - imgMeanZ[idtemp[pid]]) < THR && imgNumZ[idtemp[pid]] > 3
...
int GetGndPos(float* pos, float* fPoints, int pointNum) {
    ...
    int gndnum =
        CalGndPos_cor(tmpPos, fPoints3, pnum3, 1.0); // 用法向量判断，获取到法向量 & 地面搜索点，放到tmppos
    if (gnd_pos[5] == 0) {
