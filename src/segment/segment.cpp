...
PCSeg::PCSeg() {
    this->posFlag = 0;
    this->pVImg =
        (unsigned char*)calloc(DN_SAMPLE_IMG_NX * DN_SAMPLE_IMG_NY * DN_SAMPLE_IMG_NZ, sizeof(unsigned char));
    this->corPoints = nullptr;
}
PCSeg::~PCSeg() {
    if (this->pVImg != nullptr) {
        free(this->pVImg);
    }
    if (this->corPoints != nullptr) {
        free(this->corPoints);
    }
}
...
int PCSeg::DoSeg(int* pLabel1, float* fPoints1, int pointNum) {
    ...
    if (this->pVImg == nullptr) {
        this->pVImg = (unsigned char*)calloc(
            DN_SAMPLE_IMG_NX * DN_SAMPLE_IMG_NY * DN_SAMPLE_IMG_NZ, sizeof(unsigned char));
    }
    memset(
        pVImg, 0,
        sizeof(unsigned char) * DN_SAMPLE_IMG_NX * DN_SAMPLE_IMG_NY * DN_SAMPLE_IMG_NZ); // 600*200*30
    ...
    for (int pid = 0; pid < pointNum; pid++) {
        ...
        int iz = (fPoints1[pid * 4 + 2] + DN_SAMPLE_IMG_OFFZ) / DN_SAMPLE_IMG_DZ; // 认为地面为-1.8？
                                                                                  // -2.5~17.5
        ...
        {
            idtrans1[pid] =
                iz * DN_SAMPLE_IMG_NX * DN_SAMPLE_IMG_NY + iy * DN_SAMPLE_IMG_NX + ix; // 记录这个点对应的索引
...
int FilterGndForPos(float* outPoints, float* inPoints, int inNum) {
    ...
    for (int pid = 0; pid < inNum; pid++) {
        if (idtemp[pid] > 0) {
            ...
            if ((imgMaxZ[idtemp[pid]] - imgMinZ[idtemp[pid]]) < THR && imgMeanZ[idtemp[pid]] < -1.5
    ...
    for (int pid = 0; pid < pointNum; pid++) {
        if ((nNum < 1000) && (pLabel[pid] == 0)) {
            ...
            if (GetNeiborPCA(npca, cloud, kdtree, searchPoint, fSearchRadius) > 0) {
                ...
                // 地面向量筛选
                if ((npca.eigenValuesPCA[1] > 0.2) && (npca.eigenValuesPCA[0] < 0.1) && searchPoint.x > 29.9
                    && searchPoint.x < 40) //&&searchPoint.x>19.9
                {
                    if ((npca.eigenVectorsPCA(2, 0) > 0.9) | (npca.eigenVectorsPCA(2, 0) < -0.9)) {
                        if (npca.eigenVectorsPCA(2, 0) > 0) {
...
int GetNeiborPCA(
    SNeiborPCA& npca, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::KdTreeFLANN<pcl::PointXYZ> kdtree,
...
int PCSeg::CorrectPoints(float* fPoints, int pointNum, float* gndPos) {
    ...
    for (int pid = 0; pid < pointNum; pid++) {
        ...
        tmp[2] = RTM[2] * fPoints[pid * 4] + RTM[5] * fPoints[pid * 4 + 1] + RTM[8] * fPoints[pid * 4 + 2]
               - gndHeight; // 将地面高度变换到0 sy
...
int SegBG(
    int* pLabel, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::KdTreeFLANN<pcl::PointXYZ>& kdtree,
    float fSearchRadius) {
...
SClusterFeature FindACluster(
    int* pLabel, int seedId, int labelId, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
    pcl::KdTreeFLANN<pcl::PointXYZ>& kdtree, float fSearchRadius, float thrHeight) {
    // 初始化种子
    std::vector<int> seeds;
...
int GndSeg(int* pLabel, float* fPoints, int pointNum, float fSearchRadius) {
    ...
    for (int pid = 0; pid < pointNum; pid++) {
        if (tmpLabel1[pid] >= 0) {
            if (pGndImg1[tmpLabel1[pid]] + 0.5
                > fPoints[pid * 4 + 2]) // 相对高度限制 即pid所在点对应的格子里面高度不超过0.5，标记为1
            {
                {
                    pLabel[pid] = 1;
                    pnum++;
    ...
    for (int pid = 0; pid < pointNum; pid++) {
        if (pLabel[pid] == 1) {
            if (fPoints[pid * 4 + 2]
                > 1) // for all cases 即使这个点所在格子高度差小于0.5，但绝对高度大于1，那也不行
            {
                pLabel[pid] = 0;
            } else if (
                fPoints[pid * 4] * fPoints[pid * 4] + fPoints[pid * 4 + 1] * fPoints[pid * 4 + 1]
            ...
            if (fPoints[pid * 4] * fPoints[pid * 4] + fPoints[pid * 4 + 1] * fPoints[pid * 4 + 1]
    ...
    for (int pid = 0; pid < pointNum; pid++) {
        if (pLabel[pid] == 1
            && fPoints[pid * 4] * fPoints[pid * 4] + fPoints[pid * 4 + 1] * fPoints[pid * 4 + 1] < 400) {
...
SClusterFeature CalOBB(float* fPoints, int pointNum) {
    ...
    for (int ii = 0; ii < 180; ii++) {
        ...
        for (int pid = 0; pid < pointNum; pid++) {
            float val =
                fPoints[pid * 4] * coef_as[ii]
                + fPoints[pid * 4 + 1]
                      * coef_bs[ii]; // x*cos + y*sin 这是把每个点旋转一下，然后求旋转后点云的xy最大最小值 sy
            if (a_min > val)
                a_min = val;
            if (a_max < val)
                a_max = val;
